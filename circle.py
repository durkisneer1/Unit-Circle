...
class UnitCircle:
    ...
    def update_degrees(self, dt):
        self.current_degrees += 1 / dt
        if self.current_degrees >= 360:
            self.current_degrees = 0

    def update_cos(self):
        new_x_pos = cos(self.current_degrees)
        self.pos.x = (new_x_pos * 200) + (WIDTH / 2)
        pg.draw.line(self.screen, "red", (self.pos.x, 0), (self.pos.x, HEIGHT), 2)

    def update_sin(self):
        new_y_pos = sin(self.current_degrees)
    ...
    def draw_radius(self):
        pg.draw.line(self.screen, "black", self.center, self.pos, 2)
